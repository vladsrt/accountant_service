...
async def test_sync_role_metadata_hwm_extraction(
    ...
    meta_path.write_text(json.dumps({
        "permanentStorageHwmDate": hwm_date_str,
        "isTruncated": False
    }))
    ...
    with patch("app.services.fa3_parser.FA3Parser.parse", return_value={"ksef_reference_number": "1234"}):
    ...
    assert filters.date_type.value == "permanent_storage" if hasattr(filters.date_type, 'value') else filters.date_type == "permanent_storage"
...
async def test_sync_role_truncation_loop(
    ...
    meta_page1.write_text(json.dumps({
        "isTruncated": True,
    ...
    meta_page2.write_text(json.dumps({
        "isTruncated": False,
...
async def test_sync_company_hwm_update(
    ...
    with patch("app.services.ksef_auth.CryptoUtil.decrypt", return_value="plain_token"):
        ...
        with patch("app.services.invoice_sync.Client") as mock_client_cls:
            ...
            with patch.object(sync_service, "_sync_role", side_effect=[
                ([], seller_hwm),
                ([], buyer_hwm),
            ]):
